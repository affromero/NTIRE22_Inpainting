...
import random
from stylegan2 import PixelNorm, EqualLinear, ConvLayer, StyledConv,ToRGB
import numpy as np
...
class G_synthesis_co_mod_gan(nn.Module):
    def __init__(
        ...
        class E_fromrgb_segm(nn.Module): # res = 2..resolution_log2
            def __init__(self, res, segm_classes):
                super().__init__()
                self.FromRGB = ConvLayer(
                        num_channels+1+segm_classes,
                        nf(res-1),
                        1,
                        blur_kernel=resample_kernel,
                        activate=True)
        ...
        for res in range(resolution_log2, 2, -1):
            if res == resolution_log2:
                Es.append(
                        (
                            '%dx%d_0' % (2**res, 2**res),
                            E_fromrgb_segm(res, segm_classes)
                            ))
            Es.append(
                    (
                        '%dx%d' % (2**res, 2**res),
    ...
    def forward(self, images_in, oh_segm, masks_in, dlatents_in):
    # def forward(self, images_in, masks_in, dlatents_in):
        y = torch.cat([masks_in - 0.5, images_in * masks_in, oh_segm - 0.5], 1)
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        output, E_features = self.G_synthesis(images_in, oh_segm, masks_in, dlatent)
        # output, E_features = self.G_synthesis(images_in, masks_in, dlatent)
        return output, dlatent, E_features
