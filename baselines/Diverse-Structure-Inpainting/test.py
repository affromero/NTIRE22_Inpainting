...
folder_path = os.path.join(args.checkpoints_dir)

if os.path.isdir(folder_path) is False:
    os.mkdir(folder_path)

# Test data list
img_list = sorted([i for i in glob(os.path.join(args.img_flist, '*.png')) if 'mask' not in i])
mask_list = sorted([i for i in glob(os.path.join(args.img_flist, '*.png')) if 'mask' in i])
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    for i in range(len(img_list)):
        ...
        cv2.imwrite(os.path.join(folder_path, filename), output[:,:,::-1])
        print('%s is generated. time: %.2fs.' % (filename, time.time() - begin))
        sys.stdout.flush()
