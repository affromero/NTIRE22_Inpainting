...
if __name__=='__main__':
    ...
    img_list = sorted([i for i in glob(os.path.join(opts.image_url, '*png')) if 'mask' not in i])
    mask_list = sorted([i for i in glob(os.path.join(opts.image_url, '*png')) if 'mask' in i])
    ...
    if opts.BERT:
        ...
        for x_name,y_name in zip(img_list,mask_list):
            ...
            input_mask=Image.open(y_name).convert("L")
            ...
            input_image=Image.open(x_name).convert("RGB")
            ...
            img_name=os.path.basename(x_name)[:-4]+'.png'
