...
def load_image(fname, mode='RGB', return_orig=False, crop=None):
...
class InpaintingDataset(Dataset):
    def __init__(self, datadir, img_suffix='.jpg', crop_size=None, pad_out_to_modulo=None, scale_factor=None, decimate=False):
        self.datadir = os.path.abspath(datadir)
        if decimate:
            self.mask_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*0_mask.png'), recursive=True)))
            self.img_filenames = sorted([i for i in list(glob.glob(os.path.join(self.datadir + '_gt', '**', '*0.png'), recursive=True))])
        else:
            self.mask_filenames = sorted(list(glob.glob(os.path.join(self.datadir, '**', '*_mask.png'), recursive=True)))
            self.img_filenames = sorted([i for i in list(glob.glob(os.path.join(self.datadir + '_gt', '**', '*.png'), recursive=True)) if '_mask' not in i and '_segm' not in i])
    ...
    def __getitem__(self, i):
        image = load_image(self.img_filenames[i], mode='RGB', crop=self.crop_size)
        mask = load_image(self.mask_filenames[i], mode='L', crop=self.crop_size)
