...
class InpaintingTrainDataset(Dataset):
    def __init__(self, indir, mask_generator, transform, img_suffix='.png'):
        # self.in_files = list(glob.glob(os.path.join(indir, '**', '*.jpg'), recursive=True))
        # if 'Places' in indir:
        #     self.in_files = sorted(list(glob.glob(os.path.join(indir, '**', '**', '*'+img_suffix), recursive=True)))
        #     self.mask = sorted(list(glob.glob(os.path.join(indir + '_mask', '**', '**', '*'+img_suffix), recursive=True)))
        # else:
        # img_suffix: .JPEG # imagenet
        # img_suffix: .jpg # places and wikiart
        # img_suffix: .png # ffhq
        if 'FFHQ' in indir:
            img_suffix = '.png'
        elif 'ImageNet' in indir:
            img_suffix = '.JPEG'
        elif 'Places' in indir or 'WikiArt' in indir:
            img_suffix = '.jpg'
        self.in_files = sorted(list(glob.glob(os.path.join(indir, '**', '*'+img_suffix), recursive=True)))
        self.in_files = [i for i in self.in_files if os.path.isfile(i)]
        self.mask = sorted(list(glob.glob(os.path.join(indir + '_mask', '**', '*'+img_suffix), recursive=True)))
        self.mask = [i for i in self.mask if os.path.isfile(i)]
    ...
    def __getitem__(self, item):
        ...
        while True:
            ...
            mask = mask.resize(([img.shape[-2], img.shape[-1]]), Image.NEAREST)
            mask = TF.to_tensor(np.array(mask))
            # mask = TF.to_tensor(cv2.imread(self.mask[item])[...,0])
            # mask = TF.center_crop(mask, [img.shape[-2], img.shape[-1]])
            # mask = self.mask_generator(img, iter_i=self.iter_i)
            if mask.mean() < 1 and mask.mean() > 0:
                break
            item = random.choice(range(len(self.in_files)))
        self.iter_i += 1
